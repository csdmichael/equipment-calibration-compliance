    ...
    <ion-app>
      ...
      <ion-content class="ion-padding">
        ...
        <ng-template #detail>
          ...
          <ng-template #empty><p>Select a compliance from the list.</p></ng-template>
        </ng-template>
      </ion-content>
    </ion-app>
  `,
...
export class AppComponent {
  readonly title = "Equipment Calibration Compliance";
  readonly screens = ["1", "2", "3", "4", "Calibration Due List", "Interaction Details"];
  readonly screen = signal("1");
  readonly items = signal<Compliance[]>([]);
  readonly selected = signal<Compliance | null>(null);
  readonly error = signal('');
  private readonly http = inject(HttpClient);

  constructor() {
    this.http.get<Compliance[]>(apiUrl('/api/compliances')).subscribe({
      next: (items) => this.items.set(items),
      error: () => this.error.set('Could not reach the API.'),
    });
  }

  select(item: Compliance): void {
